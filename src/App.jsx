...
export default function App() {
  ...
  const [safeTilesToFlip, setSafeTilesToFlip] = useState(() => countSafeTiles(board));

  function countSafeTiles(board) {
    return board.flat().filter(t => t.value !== 'V').length;
  }
  ...
  const flipTile = (i, j) => {
    ...
    if (tile.value === 'V') {
      ...
      const newSafeLeft = safeTilesToFlip - 1;
      const newScore = score * tile.value;
      setScore(newScore);
      if (newScore > highScore) setHighScore(newScore);

      setSafeTilesToFlip(newSafeLeft);
      ...
      if (newSafeLeft === 0) {
        ...
        setBoard(newBoard);
        setScore(1);
        setSafeTilesToFlip(countSafeTiles(newBoard));
